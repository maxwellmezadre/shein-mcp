...
- Ids (`billno`, pacote, rastreio, `goods_sn`, `sku_code`, `store_code`, `memberId`…)
  são remapeados por `sha256(salt + valor)` mantendo prefixo e comprimento, e trocados
  no arquivo inteiro, então as relações sobrevivem: o `billno` do detalhe continua
...
- PII (nome, telefone, endereço, CEP, CPF, cidade, e-mail) é trocada por
