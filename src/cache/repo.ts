...
import { createCacheAnalytics } from "./analytics.js";
...
export function createCacheRepo(db: Database, now: () => number) {
  ...
  // The analytical queries live next door; callers still see one object.
  const analytics = createCacheAnalytics(db);
